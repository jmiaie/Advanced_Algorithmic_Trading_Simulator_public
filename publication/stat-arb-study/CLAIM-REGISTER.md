# CLAIM-REGISTER.md — D10-B `stat-arb-study`

Every claim this pack is permitted to make, classified, with its evidence and its *reversal condition*
...
- **NOT-CLAIMED** — deliberately not asserted in this pack (and why).

| ID | Claim as stated in the pack | Class | Evidence | Reversal condition |
|----|------------------------------|-------|----------|--------------------|
...
| CL-18 | The binding 2025 label is FINAL 2025 WALK-FORWARD EVALUATION; 5/19 symbols overlap the executed FDM D9-A universe; design-origin chronology is unresolved. | SUPPORTED-LIMITED | C-36, C-37; gate Field 14 | Resolution of the chronology question (would strengthen, not weaken, the disclosure) |
...
| CL-20 | Superseded lineage is preserved and unedited: the v1 result artifacts (3 files) and the v3 result artifacts (3 JSON + window-boundaries CSV) remain in `results/historical_oos/` under their original labels; the v2 lineage is **config-only** (v2 produced no result artifacts before invalidation) plus one `statarb_hist_etf_wf_v2_INVALIDATED` ledger row; three explicit `…SUPERSEDED_*` ledger rows remain. No superseded artifact, label or ledger row was altered. | SUPPORTED | C-20, C-38, C-40 | Any superseded file, label or ledger row differing from the accepted record |
| CL-21 | The strategy is *not* claimed to have a validated edge, an alpha, a Sharpe, an economic benefit, or capacity — no such quantity is estimated anywhere in this pack. | **NOT-CLAIMED** | Field 9 (evidence class); Field 13; gate prohibitions | A future authorized study producing those measurements |
| CL-22 | The 2025 result is *not* claimed to imply the method is unprofitable in general, nor that the design generalizes to other universes, periods or asset classes. | **NOT-CLAIMED** | Field 9; Field 13 items 1–4 | A pre-registered, non-overlapping multi-universe study |
| CL-23 | Nothing in this pack is claimed to constitute investment advice, a solicitation, or a performance representation. | **NOT-CLAIMED** | Pack scope (publication layer only) | n/a — scope boundary |

## Register-level notes
1. **No claim in this register depends on a number that is absent from `RESULT-SOURCE-MAP.md`.**
2. **No claim restates a gate value as an original finding.** Gate-supplied values are labeled ACCEPTED in
   `SOURCE-GATE.md` and are only used as identifiers/provenance.
3. **CL-13 is the only INTERPRETATION**, and it is deliberately narrow: it asserts *how the null arose*
   ...
   favourable. If a reviewer prefers the strictly descriptive wording, deleting CL-13 changes no other claim.
4. **The pack's headline is the null itself** (CL-02 + CL-03 + CL-06). This is the D9-B accepted result and is
