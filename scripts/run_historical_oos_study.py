...
Examples:
  python scripts/run_historical_oos_study.py \\
    --config configs/experiments/statarb_historical_oos_study_v1.yaml

  # Holdout (2025) only after YAML status is frozen-for-holdout
  python scripts/run_historical_oos_study.py --allow-holdout --skip-dev --skip-validation
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/statarb_historical_oos_study_v1.yaml"),
    )
    ...
    parser.add_argument("--branch", default="research/historical-oos-study")
    ...
    sector_grouping = dict(manifest.get("sector_grouping") or {})
    ...
    for experiment_id, form_period, eval_period in jobs:
        ...
        _append_ledger(
            ledger_path,
            {
                ...
                "primary_symbol": (
                    payload.get("selection", {}).get("top_pairs", [{}])[0].get("symbol_a", "")
                    if payload.get("selection", {}).get("top_pairs")
                    else ""
                ),
        ...
        print(
            ...
            f"rejected={key_metrics.get('n_rejected_within_sector')} "
            f"mean_sharpe={key_metrics.get('mean_sharpe')}"
        )
