...
for the full record); the dataset, universe, grid, costs, and hypotheses
are otherwise identical to v3 -- no re-acquisition needed.

Without --allow-holdout, 2025 rows are sliced out of the panel before the
study ever runs -- not just gated by a status check -- so the holdout
period is structurally unreachable during DEV/2024-validation execution.

Examples:
  # Development (2015-2023) + validation (2024); holdout structurally excluded
  python scripts/run_statarb_wf_v4_study.py

  # Holdout (2025) only after YAML status is frozen-for-holdout
  python scripts/run_statarb_wf_v4_study.py --allow-holdout
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/statarb_historical_etf_wf_v4.yaml"),
    )
    ...
    if args.allow_holdout:
        ...
        print(
            f"DEV+2024-validation only: panel sliced to {eval_panel.index.max().date()} "
            "(2025 rows excluded)."
        )
    ...
    for period_name, windows in buckets:
        ...
        experiment_id = f"statarb_hist_etf_wf_v4_{period_name}"
