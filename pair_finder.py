...
class PairFinder:
    ...
    def find_pairs(
        ...
        for sym_a, sym_b in combinations(symbols, 2):
            try:
                ...
                logger.debug("Value error testing %s/%s: %s", sym_a, sym_b, e)
            except Exception as e:
                logger.error("Unexpected error testing %s/%s", sym_a, sym_b, exc_info=True)
    ...
    def analyze_pair(self, sym_a: str, sym_b: str) -> Dict:
        ...
        try:
            ...
            return {"error": "Cointegration test failed (e.g. constant series)"}
