...
### --- PART 1: PROJECT MANUAL & DOCUMENTATION ---

#### 1. PROJECT OVERVIEW
This project is an advanced Event-Driven Backtester designed to simulate high-frequency trading mechanics. Unlike standard backtesters that assume fills at the "Close" price, this simulator models:
- Limit Order Books (LOB): Simulates depth, liquidity, and bid/ask spreads.
- Market Physics: Models slippage, commission, transaction costs, and queue position.
- Computational Optimization: Uses `numba` to bypass the Python GIL for high-speed matching.

#### 2. ARCHITECTURE
- execution.py: Contains the physics (Limit Order Book, Transaction Costs, Matching Engine).
- strategies.py: Contains the trading logic (Pairs Trading, Momentum).
- engine.py: Manages the event loop and time-series data streaming.
- analytics.py: Generates performance metrics (Sharpe Ratio, Drawdown, Equity Curve).
- main.py: The entry point that generates synthetic data and runs the simulation.

#### 3. MATHEMATICAL MODEL: PAIRS TRADING STRATEGY
The core strategy implemented is Statistical Arbitrage based on Co-integration and Mean Reversion.

##### A. The Spread Equation
   S_t = P_t^A - beta * P_t^B
   (Where beta is the hedge ratio calculated via OLS regression)

##### B. Z-Score Normalization
   Z_t = (S_t - mean(S)) / std(S)

##### C. Trading Logic
   - Entry: If |Z_t| > 2.0, bet on convergence (Short high leg, Long low leg).
   - Exit: If |Z_t| < 0.5, close the position.

#### 4. Questions & Answers

Q1: Why did you use Numba?
A1: Python loops are slow due to the GIL and type-checking. I used Numba (@jit) to compile the matching engine loop to machine code (LLVM), achieving C++ level performance for order book iteration.

Q2: How does your simulator handle Look-Ahead Bias?
A2: By using an Event-Driven Architecture. The simulator processes data tick-by-tick. The strategy only receives the market event for time T; it has zero knowledge of T+1.

Q3: How would you deploy this?
A3: The application is Dockerized. It can be deployed to AWS ECS or Kubernetes. For optimization, I would run parallel containers for Grid Search on AWS Batch.

### ============================================================
### --- PART 2: ONE-CLICK INSTALLER SCRIPT (PYTHON) ---
### ============================================================
Copy the python code below into a file named 'install_project.py' and run it.
It will generate all the necessary files (execution.py, strategies.py, Dockerfile, etc.) automatically.
